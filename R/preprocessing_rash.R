#' @title Preprocess data for Rash prediction
...
#' @param adae dataset 3
#' @param num_train number of training data
#' @importFrom dplyr select filter mutate mutate_if left_join distinct
#' @importFrom mltools one_hot
#' @importFrom data.table as.data.table
#' @importFrom stats na.omit
#' @export

preprocessing_rash <- function(info, biomark, adae, num_train) {
  info = info |>
  ...
  feature_dth_test_norm = feature_dth[, -1][(num_train+1):931]
  ...
  label_rash_train=rash[1:num_train, 2]
  label_rash_test=rash[(num_train+1):931, 2]

  outputs = list(x_train = feature_dth_train_norm, x_test = feature_dth_test_norm,
                 y_train = label_rash_train, y_test = label_rash_test)
  return(outputs)
}
